...
Beyond its <ToolCount /> tools, `cks-mcp` exposes every active session and its version
...
See [Tools Reference](../tools/index.mdx) for the tool-based equivalents of
each of these reads, and [Prompts](prompts.mdx) for the other MCP-native
