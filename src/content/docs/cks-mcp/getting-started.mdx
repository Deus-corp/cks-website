...
**Canonical Knowledge Structure** through <ToolCount /> tools, instead of holding
that knowledge loosely in its own context. See [the overview](index.mdx)
...
3. Save and fully restart Claude Desktop (quit, then reopen). A connector
   icon for `cks-mcp` (<ToolCount /> tools) should appear.
...
[Tools Reference](tools/index.mdx).
...
- [Tools Reference](tools/index.mdx) — every tool, grouped by function.
- [Architecture](architecture/ARCHITECTURE.mdx) — how the server is put
