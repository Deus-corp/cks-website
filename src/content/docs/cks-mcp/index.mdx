...
It exposes **<ToolCount /> tools** over MCP, backed by the deterministic, immutable
...
the rest of the ecosystem. See [Architecture](architecture/ARCHITECTURE.mdx)
...
| Page | Covers |
|------|--------|
| [Getting Started](getting-started.mdx) | Install, connect to Claude Desktop, first session |
| [Tools Reference](tools/index.mdx) | All <ToolCount /> tools, grouped by function, with request/response examples |
| [Architecture](architecture/ARCHITECTURE.mdx) | Layering, components, and the [request lifecycle](architecture/request-lifecycle.md) |
...
| [MCP Resources](protocol/resources.mdx) & [Prompts](protocol/prompts.mdx) | The two MCP-native features the server exposes beyond its <ToolCount /> tools |
| [ADRs](adr/) | Why specific architectural decisions were made — thin translator, provenance signing, middleware, extension model |

## Learning Path

1. Read [Getting Started](getting-started.mdx) and connect the server to
   Claude Desktop (or any MCP client).
2. Skim the [Tools Reference](tools/index.mdx) overview table to see what's
...
3. If you're extending or embedding the server rather than just using it,
   read [Architecture](architecture/ARCHITECTURE.mdx) and its
