...
[Tools Reference](../tools/index.mdx) instead.
...
Each of the <ToolCount /> tools lives in its own package under `tools/<name>/`
(a small number of packages hold two closely related tools, e.g.
`tools/revert/` has both `list_versions` and `revert_version`) — see
[Tools Reference](../tools/index.mdx) for the full, grouped list with
...
workflows as templated Prompts. See [Resources](../protocol/resources.mdx)
and [Prompts](../protocol/prompts.mdx) — these are documented nowhere else,
